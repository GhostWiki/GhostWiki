...
@if (user()->hasAppAccess())
    ...
    {{-- @if(userCanOnAny('view', \BookStack\Entities\Models\Category::class) || userCan('category-view-all') || userCan('category-view-own'))
        <a href="{{ url('/shelves') }}"
           data-shortcut="shelves_view">@icon('bookshelf'){{ trans('entities.shelves') }}</a>
    @endif --}}
    <a href="{{ url('/category') }}" data-shortcut="category_view">@icon('books'){{ trans('entities.books') }}</a>
